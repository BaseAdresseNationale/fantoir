const {keyBy, groupBy, sortBy, flatten, deburr} = require('lodash')
...
class InMemoryDatabase {
  ...
  search(text) {
    ...
    return sortBy(flatten(this._fuzzyset.get(normalizedText, [], 0.5).map(([score, matchingEntry]) => {
      return this._labelIndex[matchingEntry].map(record => {
        const retainedScore = normalizedText.includes(record.mot_directeur) ? score : score * 0.7
        return {
          ...record,
          score: retainedScore
        }
      })
    })).filter(r => r.score >= 0.5), 'score').reverse()
  }
